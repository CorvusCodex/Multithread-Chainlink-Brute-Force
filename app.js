...
process.title = "Multithread chainlink Brute Force by CorvusCodex";
...
data.toString().split("\n").forEach(address => {
    if (address.startsWith('0x')) {
        ...
        console.error('Error: chainlink addresses are not in correct format. Addresses must start with 0x');
        process.exit(1);
    }
});
...
function generate() {
    ...
    // Generating a new random chainlink keypair
